      ...
      switch (data.appointmentType) {
        case 'AC_REJUVENATION':
          await tx.acRejuvenationDetails.create({
            data: {
              ...
              lastServiceDate: data.acRejuvenationDetails.lastServiceDate ? new Date(data.acRejuvenationDetails.lastServiceDate) : null,
              refrigerantType: data.acRejuvenationDetails.refrigerantType ? toRefrigerantTypeEnum(data.acRejuvenationDetails.refrigerantType) : null,
              issues: data.acRejuvenationDetails.issues,
              currentPerformance: data.acRejuvenationDetails.currentPerformance ? toPerformanceRatingEnum(data.acRejuvenationDetails.currentPerformance) : null,
            },
          });
          break;
        ...
        case 'REPAIR_OR_REPLACE':
          await tx.repairReplaceDetails.create({
            data: {
              appointmentId: baseAppointment.id,
              systemType: toHVACSystemTypeEnum(data.repairReplaceDetails.systemType),
              ...
              budgetRange: data.repairReplaceDetails.budgetRange ? toBudgetRangeEnum(data.repairReplaceDetails.budgetRange) : null,
              preferredSolution: data.repairReplaceDetails.preferredSolution ? toPreferredSolutionEnum(data.repairReplaceDetails.preferredSolution) : null,
            },
          });
          break;
        ...
        case 'REPAIR_AND_TUNE_UP':
          await tx.repairTuneUpDetails.create({
            data: {
              appointmentId: baseAppointment.id,
              systemType: toHVACSystemTypeEnum(data.repairTuneUpDetails.systemType),
              lastTuneUpDate: data.repairTuneUpDetails.lastTuneUpDate ? new Date(data.repairTuneUpDetails.lastTuneUpDate) : null,
              specificConcerns: data.repairTuneUpDetails.specificConcerns,
              noiseLevel: data.repairTuneUpDetails.noiseLevel ? toNoiseLevelEnum(data.repairTuneUpDetails.noiseLevel) : null,
              energyEfficiency: data.repairTuneUpDetails.energyEfficiency ? toEfficiencyRatingEnum(data.repairTuneUpDetails.energyEfficiency) : null,
            },
          });
          break;
        ...
        case 'WATER_QUALITY_SOLUTIONS':
          await tx.waterQualityDetails.create({
            data: {
              appointmentId: baseAppointment.id,
              propertyType: toPropertyTypeEnum(data.waterQualityDetails.propertyType),
              waterSource: toWaterSourceEnum(data.waterQualityDetails.waterSource),
              waterIssues: toWaterIssueEnum(data.waterQualityDetails.waterIssues),
              hasWaterSoftener: data.waterQualityDetails.hasWaterSoftener,
              hasFilterSystem: data.waterQualityDetails.hasFilterSystem,
              numberOfBathrooms: data.waterQualityDetails.numberOfBathrooms,
            },
          });
          break;
        ...
        case 'INDOOR_AIR_QUALITY':
          await tx.indoorAirQualityDetails.create({
            data: {
              ...
              symptoms: toAirQualitySymptomEnum(data.indoorAirQualityDetails.symptoms),
              ...
              occupantsWithAllergy: data.indoorAirQualityDetails.occupantsWithAllergy,
...
export const getDashboardStats = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const [total, pending, confirmed, inProgress, completed, cancelled, todayAppointments] = await Promise.all([
